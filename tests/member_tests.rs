...
async fn test_update_member() {
    ...
    let req = Request::builder()
        ...
        .header("Authorization", format!("Bearer {}", token))
        .header("Content-Type", "application/json")
        .body(Body::from(r#"{"first_name":"Alicia","email":"alicia@example.com"}"#))
        .unwrap();
...
async fn test_delete_member_soft_delete() {
    ...
    let req = Request::builder()
        ...
        .header("Authorization", format!("Bearer {}", token))
        .header("Content-Type", "application/json")
        .body(Body::from(r#"{"first_name":"ToDelete","last_name":"Member"}"#))
        .unwrap();
...
async fn test_search_members() {
    ...
    for (first, last) in [("John", "Doe"), ("Jane", "Doe"), ("Bob", "Smith")] {
        let req = Request::builder()
            ...
            .header("Authorization", format!("Bearer {}", token))
            .header("Content-Type", "application/json")
            .body(Body::from(format!(r#"{{"first_name":"{}","last_name":"{}"}}"#, first, last)))
            .unwrap();
        app.request(req).await;
    }
